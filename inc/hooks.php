 ...
 */
function drift_body_classes( $classes ) {
...
add_filter( 'body_class', 'drift_body_classes' );

/**
 * Flush out the transients used in drift_categorized_blog.
 *
 * @author WebDevStudios
 *
 * @return bool Whether or not transients were deleted.
 */
function drift_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return false;
	}

	// Like, beat it. Dig?
	return delete_transient( 'drift_categories' );
}

add_action( 'delete_category', 'drift_category_transient_flusher' );
add_action( 'save_post', 'drift_category_transient_flusher' );
 ...
 */
function drift_content_more_link() {
	return ' <a class="more-link" href="' . get_permalink() . '">' . esc_html__( 'Read More', 'the-drift' ) . '...</a>';
}

add_filter( 'the_content_more_link', 'drift_content_more_link' );
 ...
 * @return string Read more link.
 */
function drift_excerpt_more( $more ) {
	return sprintf( ' <a class="more-link" href="%1$s">%2$s</a>', get_permalink( get_the_ID() ), esc_html__( 'Read more...', 'the-drift' ) );
}

add_filter( 'excerpt_more', 'drift_excerpt_more' );
 ...
 */
function drift_get_the_content( $content ) {
	return ! empty( $content ) ? $content : false;
}

add_filter( 'the_content', 'drift_get_the_content', 20 );
 ...
 */
function drift_custom_mime_types( $mimes ) {
...
add_filter( 'upload_mimes', 'drift_custom_mime_types' );
 ...
 * @author WebDevStudios
 */
function drift_include_svg_icons() {
...
add_action( 'wp_footer', 'drift_include_svg_icons', 9999 );
 ...
 */
function drift_display_customizer_header_scripts() {
	// Check for header scripts.
	$scripts = get_theme_mod( 'drift_header_scripts' );
	...
	echo drift_get_the_content( $scripts );
}

add_action( 'wp_head', 'drift_display_customizer_header_scripts', 999 );
 ...
 */
function drift_display_customizer_footer_scripts() {
	// Check for footer scripts.
	$scripts = get_theme_mod( 'drift_footer_scripts' );
...
add_action( 'wp_footer', 'drift_display_customizer_footer_scripts', 999 );
...
function drift_add_og_tags() {
	...
	$default_base_description = ( get_bloginfo( 'description' ) ) ? get_bloginfo( 'description' ) : esc_html__( 'Visit our website to learn more.', 'the-drift' );
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		$specify        = ( is_category() ) ? esc_html__( 'categorized in', 'the-drift' ) : esc_html__( 'tagged with', 'the-drift' );
		$queried_object = get_queried_object();
		$card_url       = get_term_link( $queried_object );
		$card_type      = 'website';

		// Translators: get the term name.
		$card_long_description = sprintf( esc_html__( 'Posts %1$s %2$s.', 'the-drift' ), $specify, $term_name );
	...
	if ( is_search() ) {
		...
		$card_long_description = sprintf( esc_html__( 'Search results for %s.', 'the-drift' ), $search_term );
		$card_description      = $card_long_description;
	}
...
add_action( 'wp_head', 'drift_add_og_tags' );
 ...
 */
function drift_remove_archive_title_prefix( $block_title ) {
...
add_filter( 'get_the_archive_title', 'drift_remove_archive_title_prefix' );
 ...
 * @author Corey Collins
 */
function drift_disable_wpautop_for_gutenberg() {
...
add_filter( 'init', 'drift_disable_wpautop_for_gutenberg', 9 );
