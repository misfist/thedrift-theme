 ...
 */
function drift_remove_default_customizer_sections( $wp_customize ) {
...
add_action( 'customize_register', 'drift_remove_default_customizer_sections', 15 );
 ...
 */
function drift_include_custom_controls() {
...
add_action( 'customize_register', 'drift_include_custom_controls', -999 );
 ...
 * @author WebDevStudios
 */
function drift_customize_scripts() {
	wp_enqueue_script( 'the-drift-customize-livepreview', get_template_directory_uri() . '/inc/customizer/assets/scripts/livepreview.js', [ 'jquery', 'customize-preview' ], '1.0.0', true );
}
add_action( 'customize_preview_init', 'drift_customize_scripts' );
 ...
 */
function drift_selective_refresh_support( $wp_customize ) {

	// The <div> classname to append edit icon too.
	$settings = [
		'blogname'          => '.site-title a',
		'blogdescription'   => '.site-description',
		'drift_copyright_text' => '.site-info',
	];
...
add_action( 'customize_register', 'drift_selective_refresh_support' );
 ...
 */
function drift_live_preview_support( $wp_customize ) {
...
add_action( 'customize_register', 'drift_live_preview_support', 999 );
