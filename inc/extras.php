 ...
 */
function drift_categorized_blog() {
	$category_count = get_transient( 'drift_categories' );
	...
	if ( false === $category_count ) {
		...
		set_transient( 'drift_categories', $category_count );
	}
 ...
 */
function drift_get_attachment_id_from_url( $attachment_url = '' ) {
	global $wpdb;
 ...
 */
function drift_copyright_year( $atts ) {
	// Setup defaults.
	$args = shortcode_atts(
		[
			'starting_year' => '',
...
add_shortcode( 'drift_copyright_year', 'drift_copyright_year', 15 );
 ...
 */
function drift_get_custom_logo_url() {
